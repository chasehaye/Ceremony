...
func ForgotPassword(c *gin.Context, db *gorm.DB){
    ...
    frontendURL := strings.TrimSuffix(config.App.FrontendURL, "/")
    resetLink := fmt.Sprintf("%s/reset-password/%s", frontendURL, token)
    
    if err := mail.SendResetEmail(cleanEmail, resetLink); err != nil {
...
func ResetPassword(c *gin.Context, db *gorm.DB) {
	...
	result := db.Preload("User").Where("token = ? AND used = ? AND expires_at > ?", hashedToken, false, time.Now()).Limit(1).Find(&resetRecord)
    if result.Error != nil {
